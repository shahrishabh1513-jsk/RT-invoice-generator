<div align="center">

# 🧾 RTInvoice

<img src="https://img.icons8.com/fluency/200/invoice.png" width="120"/>

### ⚡ Modern Invoice Generator Web App

![JavaScript](https://img.shields.io/badge/JavaScript-yellow)
![Frontend](https://img.shields.io/badge/Frontend-Project-blue)
![Status](https://img.shields.io/badge/Status-Active-success)

Create professional invoices instantly with a clean and modern UI 💼

</div>

---

## 🌐 Live Demo

👉https://shahrishabh1513-jsk.github.io/nova-invoice-generator/

---

## 🌟 About The Project

**NOVAInvoice** is a sleek and modern **invoice generator web application** built using **HTML, CSS, and JavaScript**.

It allows users to create, customize, and manage invoices with real-time calculations and a professional layout — making it perfect for freelancers, developers, and small businesses.

---

## ✨ Key Features

- 🧾 Dynamic invoice creation  
- ➕ Add / remove items easily  
- 💰 Real-time price calculation  
- 📊 Automatic total & tax calculation  
- 📄 Clean printable invoice layout  
- 📥 Download / print invoice  
- 📱 Fully responsive design  
- ⚡ Smooth and interactive UI  

---

## 🛠 Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=html" height="40"/>
  <img src="https://skillicons.dev/icons?i=css" height="40"/>
  <img src="https://skillicons.dev/icons?i=js" height="40"/>
</p>

---

## 🎯 Core Functionalities

- 📌 Add multiple invoice items  
- 🧮 Auto subtotal, tax & total calculation  
- 🖨️ Print invoice feature  
- 📥 Download invoice option  
- 📱 Responsive invoice preview  

---

## 📸 Screenshots

<img width="1852" height="853" alt="Screenshot 2026-06-25 203913" src="https://github.com/user-attachments/assets/c9c96ca3-701e-468a-9553-0062001403e3" />
<img width="1890" height="880" alt="Screenshot 2026-06-25 204336" src="https://github.com/user-attachments/assets/cd2dc0e9-18b6-427c-9403-858cc103507a" />
<img width="1871" height="873" alt="Screenshot 2026-06-25 204357" src="https://github.com/user-attachments/assets/9da9b33d-0f23-4201-bea9-8504b3bb26bb" />

---

## 🚀 Future Enhancements

- 🔐 User authentication  
- 📊 Invoice history tracking  
- 💾 Save invoices locally  
- 🌙 Dark mode support  
- 📧 Email invoice feature  
- 💳 Payment integration  

---

⭐ If you like this project, consider giving it a **star**!
